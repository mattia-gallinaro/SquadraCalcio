...
namespace SquadraCalcio
{
    class Squadra
    {
        ...
        public bool NomiSoloDiversi(string[] nomi, string nome)//serve per controllare che nome inserito non sia già stato inserito come nome di un'altra squadra
        {
            bool controllo = false;//variabile booleana per il controllo dei nomi
            for(int i = 0; i < nomi.Length && !controllo; i++)//ciclo for che si ripete 
            {
                 if(nomi[i] == nome)
                 {
                    controllo = true;
